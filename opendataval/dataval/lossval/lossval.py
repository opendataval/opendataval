...
from opendataval.model import TorchPredictMixin, TorchGradMixin, RegressionMLP, ClassifierMLP, GradientModel, Model
...
class LossValMLP(TorchPredictMixin, TorchGradMixin):
    ...
    Can be used for both regression and classification. Accepts the same parameters as the RegressionMLP and ClassifierMLP.
    The size of the training data set must be specified!
    """

    def __init__(self, input_dim: int, training_set_size: int, is_classification: bool, num_classes: int = 1,
                 layers: int = 5, hidden_dim: int = 25, act_fn: Optional[Callable] = None):
        """
    ...
    def fit(self,
        ...
        if val_X is None or val_y is None:  # This is necessary to enable the data addition and removal experiments!
            print("Warning: No validation data provided! Assuming Data Removal experiment and training "
                  "a standard MLP without validation.")
...
class LossValEvaluator(DataEvaluator, ModelMixin):
    def __init__(self, is_classification, loss_function: Callable=None, device=torch.device("cpu"),
                 nr_epochs=1, lr=None, mlp_args=None, *args, **kwargs):
        super(LossValEvaluator, self).__init__()
        self.args = args
        self.kwargs = kwargs
        self.device = device
        self.mlp_args = mlp_args if mlp_args is not None else {}    # MLP arguments to override the default values
        self.is_classification = is_classification
