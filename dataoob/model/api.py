...
class Classifier(Model):
    ...
    def predict(self, x):
        ...
        """
        return torch.argmax(self.predict_proba(x), dim=1)[:, None]
    def fit(self, x_train, y_train, sample_weight=None, batch_size=32, epochs=1,  verbose=False):
        ...
        for epoch in tqdm(range(int(epochs)), desc='Training Epochs'):
            ...
            for i in range(0, x_train.size(axis=0), batch_size):  # Consider dataloader, more idiomatic
                ...
                if sample_weight is not None:
                    loss = (loss * sample_weight[indices]).mean()
