...
# from tensorflow.compat.v1.keras import backend, datasets
...
def load_rossmann_data(dict_no, setting, test_store_type):
    ...
    train_data["StateHoliday"] = train_data["StateHoliday"].replace(["a", "b", "c"], 1)
    ...
    # Defines features
    data_x = data_x.drop(
        ["StoreType_a", "StoreType_b", "StoreType_c", "StoreType_d"], axis=1
    )
...
def load_image_data(data_name, dict_no, noise_rate):
    ...
    np.savez_compressed("./data_files/train.npz", x_train=x_train, y_train=y_train)
    np.savez_compressed("./data_files/valid.npz", x_valid=x_valid, y_valid=y_valid)
...
def encode_image(
    features, encoder_model, input_shape, preprocess_function, batch_size=16
):
    ...
    batch_of_images_placeholder = tf.placeholder(
        "uint8", (None, features.shape[1], features.shape[2], features.shape[3])
    )

    tf_resize_op = tf.image.resize_images(
        batch_of_images_placeholder, input_shape, method=0
    )
    ...
    def data_generator(sess, data):
        ...
        def generator():
            ...
            n = data.shape[0]
            while True:
                batch_of_images_resized = sess.run(
                    tf_resize_op, {batch_of_images_placeholder: data[start:end]}
                )
                batch_of_images__preprocessed = preprocess_function(
                    batch_of_images_resized
                )
    ...
    with tf.Session() as sess:
        ...
        ftrs_training = model.predict_generator(
            data_gen(), n_features / batch_size, verbose=1
        )

    encoded_features = np.array([ftrs_training[i].flatten() for i in range(n_features)])
