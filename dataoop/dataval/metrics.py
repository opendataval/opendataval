...
def discover_corrupted_sample(dve_out, noise_idx, noise_rate, plot=True):
    ...
    for itt in range(num_bins):
        ...
        output_perf[itt] = len(
            np.intersect1d(
                sort_idx[: int((itt + 1) * len(dve_out) / num_bins)], noise_idx
            )
        ) / len(noise_idx)
    ...
    for itt in range(num_bins):
        ...
        if len(np.unique(new_y_train)) > 1:
            ...
            if perf_metric == "auc":
                ...
                temp_output[itt, 0] = metrics.accuracy_score(
                    y_valid, np.argmax(y_valid_hat, axis=1)
                )
                temp_output[itt, 1] = metrics.accuracy_score(
        ...
        if len(np.unique(new_y_train)) > 1:
            ...
            if perf_metric == "auc":
                ...
                temp_output[num_bins + itt, 0] = metrics.accuracy_score(
                    y_valid, np.argmax(y_valid_hat, axis=1)
                )
                temp_output[num_bins + itt, 1] = metrics.accuracy_score(
                    y_test, np.argmax(y_test_hat, axis=1)
                )
...
def learn_with_dvrl(
    dve_out, eval_model, x_train, y_train, x_valid, y_valid, x_test, y_test, perf_metric
):
...
def learn_with_baseline(eval_model, x_train, y_train, x_test, y_test, perf_metric):
...
def get_metric(perf_metric: str):
